...
import cosmos from "../../Assets/Image/Cosmos__1_-removebg-preview.png";
import solana from "../../Assets/Image/solana-logo.png";

const Main: React.FC<{ onLogout: () => void }> = ({ onLogout }) => {
  ...
  return (
    <>
        ...
        <header className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="relative">
              <button onClick={() => setShowPopup(!showPopup)} className="p-2 rounded-full hover:bg-gray-200 focus:outline-none">
                <svg className="inline-block w-6 h-6 fill-n-4 transition-colors group-hover:fill-primary-1 ui-open:fill-primary-1" width="24" height="24" viewBox="0 0 24 24">
              ...
              {showPopup && (
                <div ref={popupRef} className="absolute -right-3 mt-2 w-80 bg-white shadow-lg rounded-lg p-4 z-10">
                  <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-semibold">Notifications</h2>
                    <button className="p-2 rounded-full hover:bg-gray-200 focus:outline-none">
                      <svg className="inline-block w-6 h-6 fill-n-4 transition-colors group-hover:fill-primary-1" width="24" height="24" viewBox="0 0 24 24" role="none">
                  ...
                  {notifications.map((notification) => (
                    <div key={notification.id} className="flex items-start p-2 hover:bg-gray-100 rounded-lg mb-2">
                      <div className="relative">
                        <img src={notification.imgSrc} alt={notification.name} className="w-10 h-10 rounded-full" />
                        {notification.online && (
                      ...
                      <div className="ml-4 flex-1">
                        <p className="text-sm font-medium">
                          {notification.name} <span className="font-normal">{notification.message}</span>
                        </p>
                        <p className="text-xs text-gray-500">{notification.time} · {notification.category}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="relative">
              <button onClick={() => setProfileMenuOpen(!profileMenuOpen)}>
                <img src={profile} alt="Profile" className="w-8 h-8 rounded-full aspect-square" />
              </button>
              {profileMenuOpen && (
      ...
      <div className="flex">
        ...
        <div className="w-2/6 p-6 space-y-6">
          ...
          {/* token table  */}
          <div className="  rounded-lg shadow text-black transition-colors duration-300">
      <div className="container mx-auto p-4 ">
        <div className="flex justify-between items-center ">
          <h1 className="text-lg font-bold">Crypto Prices</h1>
        </div>
        <table className="min-w-full ">
          <thead>
            <tr>
              <th className="text-left py-4 px-4 text-sm font-medium text-gray-400 border-b">Name</th>
              <th className="text-right py-4 px-4 text-sm font-medium text-gray-400 border-b ">Price</th>
            </tr>
          </thead>
          <tbody>
            {cryptos.map((crypto, index) => (
              <tr key={index} className="border-b">
                <td className="py-4 px-4 flex items-center">
                  <img src={crypto.icon} alt={crypto.name} className="max-w-6 min-h-6 mr-2 font-semibold text-gray-800" />
                  <span className="font-bold text-gray-800">{crypto.name}</span>&nbsp;&nbsp; <span className=" font-semibold text-gray-400"> {" "+(crypto.token)}</span>
                </td>
